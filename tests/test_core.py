...
from cli_weather.core.weather import fetch_weather_data, parse_weather_data, filter_best_days, save_weather_to_file, display_grouped_forecast
from cli_weather.utils import CacheManager, CLIWeatherException, choose_local_path
import requests

from cli_weather.core.location import load_locations, is_valid_location, get_location, save_location, view_locations
from cli_weather.config import load_config, save_config, CONFIG_FILE
#from cli_weather.utils import CLIWeatherException
import geopy.exc

from cli_weather.core.activity import save_activity, get_activity_criteria, view_activities, choose_activity
...
class TestWeather(unittest.TestCase):
    ...
    def test_parse_weather_data_current(self):
       current_weather = parse_weather_data(SAMPLE_WEATHER_DATA["list"][0], forecast_type="current")
    ...
    def test_parse_weather_data_hourly(self):
        ...
        self.assertEqual(len(hourly_weather), 24) # Check if it parses data for next 24 hours
    ...
    #    with patch("cli_weather.core.weather.load_config") as mock_load_config:
    ...
    @patch('cli_weather.core.weather.open', new_callable=mock_open)
    @patch('cli_weather.core.weather.choose_local_path')  # Adjusted patch path
    @patch('cli_weather.utils.confirm')
    @patch('cli_weather.utils.get_index')
    def test_save_weather_to_file(self, mock_get_index, mock_confirm, mock_choose_local_path, mock_file):
        mock_get_index.return_value = 0
        mock_confirm.return_value = True
        mock_choose_local_path.return_value = self.cache_dir

        sample_weather = [{"date": "2024-04-02", "temp": 15, "weather": "Cloudy", "wind_speed": 5, "rain": 0}]
    ...
    @patch('cli_weather.core.weather.print')  # Mock 'print' to capture output
    def test_display_grouped_forecast(self, mock_print):
        sample_forecast = [
            {'date': '2024-04-02 09:00:00', 'temp': 15, 'weather': 'Cloudy', 'wind_speed': 5, 'rain': 0},
            {'date': '2024-04-02 12:00:00', 'temp': 17, 'weather': 'Sunny', 'wind_speed': 7, 'rain': 0.5},
        ]
        display_grouped_forecast(sample_forecast, forecast_type="hourly")

        # Example assertion to check if summary is printed
        mock_print.assert_any_call("  Summary: Avg Temp: 16.00°C, Total Rain: 0.50 mm, Wind Range: 5.00-7.00 km/h")


class TestLocation(unittest.TestCase):

    @patch('cli_weather.core.location.load_config')
    ...
    @patch('cli_weather.core.location.Nominatim')
    @patch('cli_weather.core.location.requests.get')
    def test_get_location_current(self, mock_requests_get, mock_nominatim):
        ...
        mock_geolocator.reverse.return_value = None  # Simulate reverse geocoding failure
        address, lat, lon = get_location("me")
    ...
    def test_get_location_address(self, MockNominatim):
        ...
        self.assertEqual(get_location("Anytown"), ("123 Main St, Anytown", 34.56, -78.90))

        geolocator_mock.geocode.return_value = None  # Simulate location not found
        with self.assertRaisesRegex(CLIWeatherException, "Could not find location: 'Unknown Place'"):
    ...
    def test_save_location(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('cli_weather.config.CONFIG_FILE', temp_config_path):  # Patch CONFIG_FILE
                save_location("My Location", "1.23, 4.56")

                # Check the updated config
                updated_config = json.loads(temp_config_path.read_text())
                self.assertEqual(updated_config["locations"]["My Location"], "1.23, 4.56")


    @patch('builtins.print')
    @patch('cli_weather.core.location.load_locations')
...
class TestActivity(unittest.TestCase):
    ...
    @patch("cli_weather.core.activity.open", mock_open())
    def test_save_activity(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('cli_weather.config.CONFIG_FILE', temp_config_path):
                new_activity = {"temp_min": 20, "temp_max": 30, "rain": 0, "wind_min": 0, "wind_max": 10, "time_range": ["09:00", "17:00"]}
    ...
    @patch("builtins.input", side_effect=["hiking","15","25","2","10","n","y","10","y", "y"])
    def test_get_activity_criteria(self, mock_input):
        criteria = get_activity_criteria("hiking")
        self.assertEqual(criteria["temp_min"], 15)


    @patch('cli_weather.core.activity.load_config') # Mock config data
    ...
    # Reset the mock for the next test case
        ...
        view_activities()
        mock_print.assert_called_with("No activities found. Please add an activity first.")


    @patch('cli_weather.core.activity.load_config')
    @patch("builtins.input", side_effect=["1"]) # Mocking user input to choose the first option
    @patch('builtins.print') # Mock print
